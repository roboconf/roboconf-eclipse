...
import net.roboconf.tooling.core.autocompletion.InstancesCompletionProposer;
...
public class RoboconfInstancesCompletionProcessor implements IContentAssistProcessor {
	...
	public ICompletionProposal[] computeCompletionProposals( ITextViewer viewer, int offset ) {
		...
		if( editedFile == null ) {
			...
			InstancesCompletionProposer proposer = new InstancesCompletionProposer( appDirectory, editedFile );
			try {
				String text = viewer.getDocument().get( 0, offset );
				proposals = proposer.findProposals( text );
				this.errorMsg = proposer.getErrorMsg();
